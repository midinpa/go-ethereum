...
func (l *txList) Filter(costLimit, gasLimit *big.Int) (types.Transactions, types.Transactions) {
	...
	if !l.gasFromValueIncluded && l.costcap.Cmp(costLimit) <= 0 && l.gascap.Cmp(gasLimit) <= 0 {
		return nil, nil
	}
	...
	removed := l.txs.Filter(func(tx *types.Transaction) bool {
		if(tx.GasFromValue()) {
			return tx.Gas().Cmp(tx.Value()) > 0 || tx.Gas().Cmp(gasLimit) > 0
		} else {
			return tx.Cost().Cmp(costLimit) > 0 || tx.Gas().Cmp(gasLimit) > 0
		}
	})
